...
namespace s2s {
    ...
    void freq_cut(const std::string file_path, dynet::Dict& d, unsigned int dim_size){
    ...
    void load_corpus(const std::string file_path, const int start, const int end, dynet::Dict& d, std::vector<Sent>& corpus){
        ifstream in(file_path);
        assert(in);
        int tlc = 0;
        int ttoks = 0;
